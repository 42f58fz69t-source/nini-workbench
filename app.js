...
const MODULES = [
  ...
  { id: 'pomodoro', icon: ICONS.grid, color: '#7e57c2', title: '专注番茄钟', desc: '25分钟专注+休息', group: 'study', builtIn: true },
...
/* ============================================
   ...
   ============================================ */
const Pomodoro = {
  _timer: null, _left: 25 * 60, _mode: 'focus', _running: false, _count: 0,
  _task: '', // 当前专注任务文本（donelist 项 text 或自定义）
  render() {
    const today = todayStr();
    const stats = DB.get('pomodoro', {});
    const todayCount = stats[today] || 0;
    const total = Object.values(stats).reduce((s, n) => s + n, 0);
    const min = Math.floor(Pomodoro._left / 60);
    const sec = Pomodoro._left % 60;
    const timeStr = `${String(min).padStart(2,'0')}:${String(sec).padStart(2,'0')}`;
    const modeLabel = Pomodoro._mode === 'focus' ? '🍅 专注中' : '☕ 休息中';
    const modeColor = Pomodoro._mode === 'focus' ? 'var(--primary)' : 'var(--warning)';

    // 今日待办（未完成）作为专注候选
    const todos = DB.get('donelist', []).filter(t => t.date === today && !t.done);
    const opts = todos.map(t => `<option ${Pomodoro._task === t.text ? 'selected' : ''}>${esc(t.text)}</option>`).join('');
    const taskStats = DB.get('pomodoroTasks', {});
    const sortedTasks = Object.entries(taskStats).sort((a,b) => b[1]-a[1]).slice(0,5);
    const taskListHtml = sortedTasks.length ? sortedTasks.map(([name, n]) =>
      `<div class="list-item"><div class="main"><div class="title">${esc(name)}</div></div><div class="right"><span class="badge">🍅 ${n}</span></div></div>`).join('') :
      `<div class="empty" style="padding:14px"><div class="emoji">📭</div><div>还没有专注记录</div></div>`;

    return `
      <div class="card" style="text-align:center;padding:30px 16px">
        <div style="font-size:14px;color:var(--text-2);font-weight:600;margin-bottom:8px">${modeLabel}</div>
        <div style="font-size:64px;font-weight:800;color:${modeColor};font-variant-numeric:tabular-nums;letter-spacing:-2px">${timeStr}</div>
        <div style="font-size:12px;color:var(--text-3);margin-top:4px">${Pomodoro._mode === 'focus' ? '保持专注，手机放远一点~' : '喝口水，活动一下身体~'}</div>
        <div style="display:flex;gap:10px;margin-top:24px;justify-content:center">
          <button class="btn" style="width:auto;padding:12px 32px" onclick="Pomodoro.toggle()">${Pomodoro._running ? '⏸ 暂停' : '▶ 开始'}</button>
          <button class="btn secondary" style="width:auto;padding:12px 20px" onclick="Pomodoro.reset()">↺ 重置</button>
        </div>
        <div style="display:flex;gap:8px;margin-top:16px;justify-content:center">
          <button class="btn secondary small ${Pomodoro._mode==='focus'&&Pomodoro._left===25*60?'':''}" onclick="Pomodoro.setMode('focus',25)">🍅 专注25</button>
          <button class="btn secondary small" onclick="Pomodoro.setMode('break',5)">☕ 休息5</button>
          <button class="btn secondary small" onclick="Pomodoro.setMode('focus',45)">🍅 专注45</button>
        </div>
      </div>
      <div class="card">
        <div class="card-title"><span>🎯 本次专注事项</span></div>
        <select class="input" onchange="Pomodoro.setTask(this.value)">
          <option value="">— 选择今日待办 —</option>
          ${opts}
        </select>
        <input class="input" id="pomoCustomTask" placeholder="或自定义专注事项（如：背单词30个）" value="${Pomodoro._task && !todos.some(t=>t.text===Pomodoro._task) ? esc(Pomodoro._task) : ''}" oninput="Pomodoro.setTask(this.value)" style="margin-top:8px">
        <div style="font-size:12px;color:var(--text-2);margin-top:8px;text-align:center">
          ${Pomodoro._task ? `当前：${esc(Pomodoro._task)}` : '未选择事项（开始后会计入统计）'}
        </div>
      </div>
      <div class="card">
        <div class="card-title"><span>📊 今日番茄</span></div>
        <div class="stat-row">
          <div class="stat-box"><div class="num">${todayCount}</div><div class="lbl">今日完成</div></div>
          <div class="stat-box"><div class="num">${total}</div><div class="lbl">累计完成</div></div>
          <div class="stat-box"><div class="num">${todayCount * 25}</div><div class="lbl">今日专注(分钟)</div></div>
        </div>
        <div style="font-size:11px;color:var(--text-3);text-align:center;margin-top:8px">${Pomodoro._count > 0 ? '本次已连续完成 ' + Pomodoro._count + ' 个番茄' : '开始第一个番茄吧！'}</div>
      </div>
      <div class="card">
        <div class="card-title"><span>🏆 专注事项排行</span></div>
        ${taskListHtml}
      </div>
    `;
  },
  setTask(v) { Pomodoro._task = (v || '').trim(); Nav.refresh(); },
  toggle() {
    if (Pomodoro._running) {
      clearInterval(Pomodoro._timer);
      Pomodoro._running = false;
    } else {
      if (Pomodoro._mode === 'focus' && !Pomodoro._task) {
        // 不强制，但提示：可先选事项
        PetCat.cheer('开始啦！可在上方选一个专注事项~ 🐾');
      }
      Pomodoro._running = true;
      Pomodoro._timer = setInterval(() => {
        Pomodoro._left--;
        if (Pomodoro._left <= 0) {
          clearInterval(Pomodoro._timer);
          Pomodoro._running = false;
          if (Pomodoro._mode === 'focus') {
            Pomodoro._count++;
            const t = todayStr();
            const stats = DB.get('pomodoro', {});
            stats[t] = (stats[t] || 0) + 1;
            DB.set('pomodoro', stats);
            // 累计到专注事项
            if (Pomodoro._task) {
              const tk = DB.get('pomodoroTasks', {});
              tk[Pomodoro._task] = (tk[Pomodoro._task] || 0) + 1;
              DB.set('pomodoroTasks', tk);
              PetCat.cheer(`「${Pomodoro._task}」+1个番茄！🍅 休息一下`);
            } else {
              PetCat.cheer('番茄完成！🍅 休息一下吧');
            }
            Pomodoro._mode = 'break';
            Pomodoro._left = 5 * 60;
          } else {
            PetCat.cheer('休息结束！开始下个番茄 💪');
            Pomodoro._mode = 'focus';
            Pomodoro._left = 25 * 60;
          }
        }
        Nav.refresh();
      }, 1000);
  ...
  reset() {
    clearInterval(Pomodoro._timer);
    Pomodoro._running = false;
    Pomodoro._left = (Pomodoro._mode === 'focus' ? 25 : 5) * 60;
    Nav.refresh();
  },
  setMode(mode, min) {
    clearInterval(Pomodoro._timer);
...
const Settings = {
  ...
  exportWeekly() {
    ...
    const studyLogs = subjects.reduce((s,sub) => s + (sub.logs||[]).filter(l => l.date >= ws && l.date <= we).length, 0);
    const pomodoro = DB.get('pomodoro', {});
    const pomoCount = Object.keys(pomodoro).filter(d => d >= ws && d <= we).reduce((s,d)=>s+pomodoro[d], 0);
    ...
    html += `<h2 style="color:#558b2f">📚 学习</h2><p>学习记录 ${studyLogs} 条 · 番茄钟 ${pomoCount} 个</p>`;
